 ...
 ; (function ($w) {
  ...
  jm.view_provider.prototype = {
      init: function () {
          ...
          // fix：mind重新赋值页面绘制多个jsmind 删除多余jsmind
          try {
              let allCascade = document.getElementsByClassName("jsmind-inner");
      ...
      setZoom: function (zoom) {
          ...
          for (var i = 0; i < this.e_panel.children.length; i++) {
              ...
              // this.e_panel.children[i].style.marginTop = zoom === 1? '0px' : (zoom * 1000) + 'px'; // lxx 缩放不超出画布
              // this.e_panel.children[i].style.marginLeft = zoom === 1 ? '0px' : (zoom * 1000 / 6) + 'px'; // lxx 缩放不超出画布

              this.e_panel.children[i].style.zoom = zoom;

          };
          this.show(false); // lxx this.show(true) // 缩放不重新定位至中心
          return true;
