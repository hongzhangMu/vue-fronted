<template>
  <admin-layout>
    ...
    />
    <div :class="['tabs-view-content', layout, pageWidth]" :style="`margin-top: ${multiPage ? -24 : 0}px;height: ${Height}`">
      <page-toggle-transition style="height: 100%" :disabled="animate.disabled" :animate="animate.name" :direction="animate.direction">
...
export default {
  ...
  computed: {
    ...
    Height() {
      return `calc(100vh - 64.5px - 40px - 53px)`
    }
  },
  watch: {
    ...
    '$route': function(newRoute) {
      ...
      } else if (this.pageList.findIndex(item => (item.fullPath === newRoute.fullPath || item.keyPath === newRoute.path)) === -1) {
        this.pageList.push(this.createPage(newRoute))
      } else if (newRoute.path === '/overallOutput') {
        // 总体产出页面
        let flag = false
        this.pageList.forEach(e => {
          if (e.keyPath === '/overallOutput') {
            e.fullPath = newRoute.fullPath
            flag = true
          }
        })
        if (!flag) {
  ...
  created() {
    ...
    if (this.pageList.findIndex(item => item.fullPath === route.fullPath || item.keyPath === route.path) === -1) {
  ...
  methods: {
     ...
     */
    setCachedKey(route) {
      const page = this.pageList.find(item => item.fullPath === route.fullPath || item.keyPath === route.path)
      if (route.path === '/basic') {
        page.fullPath = route.fullPath
      }
